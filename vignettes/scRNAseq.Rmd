...
The data we are using here is a very small sample dataset provided by 10X.  It includes 2 PBMC samples run on different 10X machines:  the standard Chromium machine and the new Chromium X.  
...
For example:
```{r, eval = FALSE, dev='png', fig.align='center', fig.width=4.5, fig.height=3.5}
...
```{r, dev='png', fig.align='center', fig.width=6.0, fig.height=3.5}
...
```{r, dev='png', fig.align='center', fig.width=4.5, fig.height=3.5}
...
```{r, dev='png', fig.align='center', fig.width=5.5, fig.height=3.5}
...
```{r, dev='png', fig.align='center', fig.width=7.0, fig.height=3.5}
bb_var_umap(vignette_cds, 
            var = "density", 
            facet_by = "equipment", 
            cell_size = 2, 
...
```{r, dev='png', fig.align='center', fig.width=7.5, fig.height=3}
...
This function also provides a way to group cells by 2 factors.  You specify which factors you want as plot facets and which as ticks on the X axis.  You have to provide a pretty verbose table in order to do this, but it is a useful feature.
...
Here "leiden_assignmentDC/Mono" means that the comparison is DC/Mono vs T/NK cells.  You know this because T/NK does not show up in the term column.  It was chosen alphabetically as the reference condition.  In this case, a positive estimate means that the gene is up in DC/Mono vs T/NK cells.
