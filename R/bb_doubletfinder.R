...
#' @export
#' @importFrom dplyr pull filter select contains full_join
#' @importFrom Seurat CreateSeuratObject NormalizeData FindVariableFeatures ScaleData RunPCA VariableFeatures FindNeighbors FindClusters RunUMAP
#' @importFrom monocle3 exprs
#' @importFrom DoubletFinder summarizeSweep find.pK modelHomotypic
#' @importFrom tibble as_tibble
bb_doubletfinder <- function(cds,
                             doublet_prediction,
                             qc_table,
                             ncores = 1) {
  keepers <- dplyr::pull(dplyr::filter(qc_table, qc.any == FALSE), barcode)
  seu_object <- Seurat::CreateSeuratObject(monocle3::exprs(cds))
  seu_object@meta.data$barcode <- rownames(seu_object@meta.data)
  seu_object <- subset(seu_object, subset = barcode %in% keepers)
  seu_object <- Seurat::NormalizeData(seu_object)
  seu_object <- Seurat::FindVariableFeatures(seu_object,
                                             selection.method = "vst",
                                             nfeatures = 2000)
  all.genes <- rownames(seu_object)
  seu_object <- Seurat::ScaleData(seu_object, features = all.genes)
  seu_object <- Seurat::RunPCA(seu_object,
                               features = Seurat::VariableFeatures(object = seu_object))
  seu_object <- Seurat::FindNeighbors(seu_object)
  seu_object <- Seurat::FindClusters(seu_object)
  seu_object <- Seurat::RunUMAP(seu_object, dims = 1:10)

  ## pK Identification (no ground-truth) ---------------------------------------------------------------------------------------
  sweep.res.list <- param_sweep(seu_object,
                                              PCs = 1:10,
                                              sct = FALSE,
                                              num.cores = ncores)
  ...
  seu_object_lowconf <-  doublet_finder(
    seu_object,
    PCs = 1:10,
    pN = 0.25,
    pK = 0.09,
  ...
  seu_object_highconf <-  doublet_finder(
    seu_object,
    PCs = 1:10,
    pN = 0.25,
    pK = 0.09,
  ...
  solm <- tibble::as_tibble(seu_object_lowconf@meta.data) |>
    dplyr::select(barcode,
                  doubletfinder_low_conf = dplyr::contains("classifications"))
  sohm <- tibble::as_tibble(seu_object_highconf@meta.data) |>
    dplyr::select(barcode,
                  doubletfinder_high_conf = dplyr::contains("classifications"))
  return(dplyr::full_join(solm, sohm))
}
