...
bb_rowmeta <- function(obj, row_name = "feature_id", assay = "RNA", cds = NULL) {
  cds_warn(cds)
  obj_stop(obj)
  if ("cell_data_set" %in% class(obj))
    res <- tibble::as_tibble(x = SingleCellExperiment::rowData(obj), rownames = row_name)
  if ("Seurat" %in% class(obj))
    res <- tibble::as_tibble(x = obj[["RNA"]][[]], rownames = row_name)
  return(res)
}
